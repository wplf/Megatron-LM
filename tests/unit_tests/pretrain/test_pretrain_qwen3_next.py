...
@pytest.mark.parametrize(
    ("tp", "sp", "pp", "ep", "cp"), [(1, False, 1, 1, 1), (2, False, 2, 2, 1), (2, True, 2, 2, 1)]
)
def test_qwen3_next_pretrain_and_resume(tmp_path_dist_ckpt, model_name, tp, sp, pp, ep, cp):
    ...
    ret = os.system(shlex.join(cmd))  # TODO(yuzhongw): use subprocess.run instead of os.system
    assert ret == 0, "Failed to run command: " + " ".join(cmd)
