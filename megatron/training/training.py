...
def pretrain(
    ...
    if args.record_memory_history and (
        ...
        # 2.4 / 2.5 / 2.6. The new-style kwargs (enabled, context, stacks)
        # are silently translated to `trace_alloc_max_entries` by some
        # wrapper versions, which the underlying C++ impl then rejects.
        # Fall back to the legacy positional form on that failure path.
        try:
