...
class CheckpointWithoutOutput(object):
    ...
    def discard_output(self):
        """
        Release the output tensor storages (keeping tensor metadata for backward).

        Use together with :meth:`register_recompute_hook` when the output must be freed in a
        different place than where the recompute hook is registered (e.g. the output is consumed
        later than the tensor whose backward must trigger the recompute). For the common case where
        both happen at once, use :meth:`discard_output_and_register_recompute`.
        """
        from megatron.core.transformer.cuda_graphs import is_graph_warmup

        # When ckpt_manager is set, this is a no-op; the manager handles discarding uniformly.
    ...
    def register_recompute_hook(self, hook_tensor):
        """
        Register the recompute function as a grad hook of ``hook_tensor``.

        When the gradient of ``hook_tensor`` is computed, the recomputation is triggered. The
        ``hook_tensor`` must be selected so that its gradient is computed before the discarded
        outputs are needed by other backward computations (and, if the recompute itself depends on
        other discarded activations, after those have been restored).
        """
        from megatron.core.transformer.cuda_graphs import is_graph_warmup
